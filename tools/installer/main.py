...
def init_client_service(client_path, client_type):
    """Initialize directory containing source code for specific client service"""
...
def init_auth_service():
    ...
    if auth_service_type == 'Build from context':
...
def dump_docker_compose_file_context(file_context, file_name):
    """Dumps given context into corresponding docker-compose file"""
    file = open(file_name, 'w+')
    yaml.dump(file_context, file, default_flow_style=False, sort_keys=False)
    file.close()
...
def main():
    ...
    initial_count = 0
    directory_path = questionary.text('Enter project path:').ask()
    if '~' in directory_path:
        console.print('Wrong path received!', style='red bold')
        exit(1)
    if directory_path == '':
        console.print('Received empty string. Choosing current directory.', style='bold')
        directory_path = '.'
    ...
    client_type = questionary.select('What build context of client you need?', choices=['Vue.js', 'Nuxt.js']).ask()
    clients_path = 'clients/'
    init_client_service(clients_path, client_type)
    Path(clients_path + 'mobile').mkdir(parents=True)
    Path(clients_path + 'mobile/.emptydir').touch()

    init_auth_service()

    while questionary.confirm('Create new service?').ask():
        service_name = questionary.text('Enter service name, like the following example: `core-service`:').ask()
        if service_name == '':
            console.print('Received Empty string. Please try again.', style='red bold')
            continue
    ...
    for service_name in user_services_local:
        ...
        docker_compose['services']['postgres']['healthcheck']['test'] \
            += ' && pg_isready --username=${DB_USERNAME} --dbname=' + get_shorten_service_name(service_name)

    docker_compose_deploy_testing['services']['client'] = {
        'build': {
            'args': {
    ...
    docker_compose_deploy_testing['services']['web-service'] = {
        'ports': ['0.0.0.0::8080'],
    }
    docker_compose_deploy_testing['services']['postgres'] = {
        'ports': ['0.0.0.0::8080'],
    }
    ...
    dump_docker_compose_file_context(docker_compose_deploy, DOCKER_COMPOSE_DEPLOY_FILE_NAME)
    dump_docker_compose_file_context(docker_compose_deploy_develop, DOCKER_COMPOSE_DEPLOY_DEVELOP_FILE_NAME)
    dump_docker_compose_file_context(docker_compose_deploy_stage, DOCKER_COMPOSE_DEPLOY_STAGE_FILE_NAME)
    dump_docker_compose_file_context(docker_compose_deploy_production, DOCKER_COMPOSE_DEPLOY_PRODUCTION_FILE_NAME)
    dump_docker_compose_file_context(docker_compose_local, DOCKER_COMPOSE_LOCAL_FILE_NAME)
    dump_docker_compose_file_context(docker_compose_deploy_testing, DOCKER_COMPOSE_DEPLOY_TESTING_FILE_NAME)

    dump_docker_compose_file_context(docker_compose, DOCKER_COMPOSE_FILE_NAME)
