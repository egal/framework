    ...
    <div v-else-if="mergedData.searchableInput && mergedData.multiple" :style="getStyleVars">
      <EInput
        ...
        :data="{
          ...
          showFilled: false,
          type: 'search',
          iconLeft: mergedData.inputConfig?.iconLeft || '',
...
export default {
  ...
  methods: {
    onEnter(event) {
      if (this.mergedData.searchableInput && this.mergedData.multiple) {
        ...
        if (this.mergedData.searchableInput && this.mergedData.multiple && event.target.value) {
          ...
          let startKey = ''
          let endKey = ''
          if (this.mergedData.shownKey.includes('.')) {
            startKey = this.mergedData.shownKey.split('.').shift()
            endKey = this.mergedData.shownKey.split('.').pop()
            obj[startKey][endKey] = event.target.value
          } else {
     ...
     */
    deleteOption(option) {
      if (Array.isArray(this.selectModel) && this.selectModel.length) {
